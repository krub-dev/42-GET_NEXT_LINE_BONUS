...
/*   Updated: 2024/10/15 21:00:53 by frubio-i         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		if (str_buffer)
		{
			...
			if (check_str_n(new_line) == 0
				...
				return (new_line);
			if (gnl_strlen_n(new_line, 0) == 0
				&& gnl_strlen_n(str_buffer, 0) == 0)
				return (NULL);
		}
		else
		{
			str_buffer = gnl_calloc_read_free(BUFFER_SIZE, 1, fd, str_buffer);
			if (gnl_strlen_n(str_buffer, 0) == 0)
